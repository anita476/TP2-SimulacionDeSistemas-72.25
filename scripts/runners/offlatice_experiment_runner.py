#!/usr/bin/env python3
"""Run repeated off-lattice simulations and aggregate VA and cluster S."""
...
def main() -> None:
    ...
    cluster_dir = output_dir / "cluster-results"
    ...
    for run in range(args.runs):
        ...
        cluster_command = [
            ...
            "--in", str(trajectory_path),
            "--out", str(cluster_path),
            "--L", str(cluster_l),
            "--rc", str(cluster_rc),
        ]
    ...
    va_output = output_dir / "va.txt"
    cluster_output = output_dir / "cluster_s.txt"
    va_output.write_text(aggregate(va_runs, "va"), encoding="utf-8")
    cluster_output.write_text(aggregate(s_runs, "s"), encoding="utf-8")
    # write run configuration to config.txt in the output directory
    cfg = {
        "model": args.model,
        "L": args.L,
        "rho": args.rho,
